...
Implemented tests cover core construction and invariants; deterministic loopback HTTP, redirect, cookie, form, compression, resource-limit, cancellation, safe-download, destination-policy, and local TLS behavior; static semantic fixtures; state transitions, ambiguity, reset, eviction, stale lookup, memory bounds, and session isolation; complete observation semantics, capabilities, bases, resets, bounds, omissions, and deterministic ordering; relevance signals, stable ties, tokenizer classifications, complete-unit/context packing, essential overshoot, omission accounting, large boilerplate, and budget properties; and authorized actions across local controls, GET/POST forms, duplicate fields, navigation, validation, errors, downloads, stale/missing refs, and sessions. Later milestones will add terminal-runnable layers for:
