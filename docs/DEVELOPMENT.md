...
Implemented tests cover core construction and invariants; deterministic loopback transport; static semantic fixtures; state transitions, ambiguity, reset, eviction, stale lookup, memory bounds, and session isolation; complete observations; semantic add/update/remove, properties, reference replacement, reorder, summary, validation, alerts, exact reconstruction, identical states, bounds, and reset fallback; relevance and budget properties including rejection of incremental input; authorized actions including a reconstructable validation delta; and deterministic batches. Later milestones will add terminal-runnable layers for:
